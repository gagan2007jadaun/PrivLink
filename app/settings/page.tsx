...
export default function Settings() {
    ...
    // Prevent hydration mismatch & Handle Slide Animation
    useEffect(() => {
        setMounted(true);
        // Start with sidebar open matching chat, then slide it out for focus mode
    ...
    return (
        <div className="flex h-screen w-full overflow-hidden bg-background text-foreground transition-colors duration-300 font-sans">
            {/* Unified Left Navigation Panel (matching /chat) with Slide-out Animation */}
            <div className={`hidden md:flex md:h-[calc(100vh-24px)] rounded-[24px] overflow-hidden shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] border border-white/20 dark:border-white/10 z-20 shrink-0 bg-sidebar transition-all duration-300 ease-in-out origin-left ${isSidebarOpen ? 'w-[384px] opacity-100 md:my-3 md:ml-3 transform-none' : 'w-0 opacity-0 -translate-x-full md:my-0 md:ml-0 md:border-0'}`}>
                <div className="flex w-[384px] shrink-0">
                    <AppNavigation />
                    <Sidebar
                        chats={mockChats}
                        activeChatId=""
            ...
            <div className="flex flex-1 flex-col py-4 px-4 items-center justify-center transition-all duration-300">
                <main className={`flex flex-1 flex-col min-w-0 rounded-3xl overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-2xl bg-white dark:bg-sidebar transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-full' : 'w-full max-w-5xl'}`}>
                ...
                <header className="flex h-16 shrink-0 items-center justify-between border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-sidebar px-6 shadow-sm backdrop-blur-md">
                    <div className="flex items-center gap-3">
                        <button onClick={handleCloseSettings} className="flex h-8 w-8 items-center justify-center rounded-lg text-zinc-500 dark:text-zinc-300 transition-colors hover:bg-black/5 dark:hover:bg-white/10 hover:text-black dark:hover:text-white">
